                ...
                try
                {
                    ...
                    if (currentCommand.Contains("msg"))
                    {
                        ...
                        try
                        {
                            if(Arguments[0] == "msg")
                            {
                                //Argument[1] хранит строку с сообщением
                                string sMess_Decrypt = Arguments[1];
                                int iIndex = sMess_Decrypt.IndexOf(":");
                                iIndex++;
                                //Проверка на нахождение
                                if (iIndex >= 0)
                                {
                                    string sMess_Encrypt = sMess_Decrypt.Substring(iIndex);
                                    //Дешифруем собщение чата

                                    try
                                    {
                                        string sMessageNormal = Decrypt(sMess_Encrypt, "Passpord11", "Password22", "SHA1", 2, "16CHARSLONG12345", 256);
                                        //Обрезаем строку с шифрованным сообщением
                                        sMess_Decrypt = sMess_Decrypt.Remove(iIndex);
                                        sMess_Decrypt = sMess_Decrypt + " " + sMessageNormal;
                                        //Возвращаем на место
                                        Arguments[1] = sMess_Decrypt;
                                    }
                                    catch (Exception exc)
                                    {
        ...
        private void messageData_KeyUp(object sender, KeyEventArgs e)
        {
            ...
            if (e.KeyData == Keys.Enter)
            {
                ...
                if (msgData[0] == '"')
                {
                    ...
                    //Тут хранится текст сообщения
                    Send($"#private|{temp}|{content}");
                }
                else
                {
                    //Шифрование строки с сообщением
                    string temp_Encypt = Encrypt(msgData, "Passpord11", "Password22", "SHA1", 2,
                                                 "16CHARSLONG12345", 256);

                    //Тут хранится текст сообщения
                    Send($"#message|{temp_Encypt}");
                }
